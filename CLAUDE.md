...
**実装**:
- **MCP 経路（優先）**: `mcp__cvi-voice__speak` tool が Python FastMCP server
  (`mcp/server.py`, `uv run --script` で起動) 経由で `say` を呼ぶ。
  Claude Code sandbox 外で動作するため `dangerouslyDisableSandbox: true` 不要
- **Bash fallback**: `scripts/post-speak.sh` + `speak-sync.sh` を `dangerouslyDisableSandbox: true`
  で起動。MCP 経路が不可能な際の defense in depth。
- 経路選択は `commands/speak.md` の指示に従い Claude 側で判定
...
- [x] `speak.md` が MCP 優先 + Bash fallback の 2 経路指示
