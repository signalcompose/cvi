...
- [x] tool signature を parrotvox に合わせる (`speak(text, voice?, rate?)`)
- [x] Lock path を `/tmp/claude/cvi/` に移動 (sandbox-writable)

### Phase 5（将来）
- [ ] GUI設定ツール（parrotvox 側で担当される可能性あり）
- [ ] プロジェクトごとの設定
- [ ] 音声ファイルのカスタマイズ
- [ ] parrotvox 移行: `speak.md` の server 指定を `mcp__cvi-voice__speak`
  → `mcp__parrotvox__speak` に差し替え
- [ ] parrotvox 安定版配布後に CVI plugin を deprecation 検討
