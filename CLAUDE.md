...
- Stop hook は音声呼び出しに加え、日本語モードではターン内のどこかに日本語の散文本文があり、かつ最後の非空行が日本語または Voice 行であることを検査する（本文と Voice 行の表示順は問わない、コードブロックのみで構成される応答はフェンスが閉じていない場合も例外として許可）
...
**実装**:
- **MCP 経路（唯一）**: `mcp__cvi-voice__speak` tool が Python FastMCP server
  (`mcp/server.py`, `uv run --script` で起動) 経由で `say` を呼ぶ。
  Claude Code sandbox 外で動作するため `dangerouslyDisableSandbox: true` 不要。
- Bash fallback は撤廃済（#242）。MCP 起動失敗時は `/cvi:check` で診断する。
...
プラグインシステム登場前の手順で、現在は MCP server + hooks + commands +
skills を全て自前で配線する必要があるため非推奨。どうしても必要な場合は
`scripts/cvi-setup` を参照。
...
CVI の speak は MCP server 経由のみ。Issue #233 で MCP 経路を追加、#242 で
Bash fallback を撤廃した。

| 経路 | 実装 | Sandbox | `dangerouslyDisableSandbox` |
|------|------|---------|-----|
| **MCP** | `mcp/server.py` (FastMCP, Python) | 外（subprocess） | 不要 |

`commands/speak.md` は `mcp__cvi-voice__speak` の呼び出しのみを指示する。
parrotvox が配布可能になったら speak.md の server 指定を `mcp__parrotvox__speak`
に差し替えるだけで移行できる設計。
...
- [x] (bash 経路のロック/PID管理統合 — #242 で経路ごと撤廃)
- [x] SessionEnd hook でのクリーンアップ
- [x] SessionStart / Notification hook の async 化
- [x] 共通 lib/config.sh による config 読み込み統合

### Phase 4: MCP server 化（完了、2026-04）
- [x] Python FastMCP ベースの `cvi-voice` MCP server を plugin 同梱 (`mcp/server.py`)
- [x] `.mcp.json` で自動登録、`uv run --script` で zero-install 化
- [x] `speak.md` が MCP 経路を指示（fallback 撤廃、#242）
- [x] 最小 tool signature `speak(text, voice?, rate?)` を CVI 独自に定義
- [x] Lock path を `/tmp/claude/cvi/` に移動 (sandbox-writable)
- [x] Bash fallback (`post-speak.sh`, `speak-sync.sh`) を撤廃（#242, 2026-04）

### Phase 5: parrotvox への置き換え（将来、CVI plugin deprecation）

CVI は interim 実装であり、[parrotvox](https://github.com/signalcompose/parrotvox)
（macOS 向け独立 TTS アプリケーション、CVI の後継として開発中）が
distribution-ready になった時点で CVI plugin を deprecation する想定。

- [ ] parrotvox 安定版の配布確認（TestFlight / notarized DMG 等）
- [ ] CVI plugin の deprecation アナウンス / README 誘導
- [ ] ユーザーは parrotvox をインストールし、parrotvox が提供する MCP server
  を直接使用する（CVI plugin の speak.md / MCP server は使われなくなる）

備考: parrotvox は Swift native で独自実装されており、CVI の tool signature
や実装詳細との互換性は前提としていない。CVI → parrotvox は**migration**
ではなく**replacement**。
