...
で構成される応答は、フェンスが閉じていない場合も意図的な例外として許可する。
...
- **MCP server** (`mcp/server.py`): Python FastMCP で `speak(text, voice?, rate?)` tool 提供（CVI 独自の最小 signature）
...
- **位置付け**: CVI は [parrotvox](https://github.com/signalcompose/parrotvox)
  （macOS 向け独立 TTS アプリ、CVI の後継）が distribution-ready になるまでの
  interim 実装。parrotvox は Swift native で独自実装されており、CVI との
  互換性は前提としない。parrotvox 完成時には CVI plugin は deprecation される。
