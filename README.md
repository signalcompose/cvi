...
**Phase 1-3の実装**（2026-02）により、以下の機能を提供：

#### Phase 1: 同期音声再生（MVP）

- **PostToolUse hook**: `/cvi:speak` Skill tool実行時に自動トリガー
- **speak-sync.sh**: 同期音声再生（`say`コマンド、フォアグラウンド実行）
- **設定読み込み**: `~/.cvi/config`から音声設定を読み込み
- **タイミング保証**: 音声完了後に「Speaking: ...」が表示
...
- **Sentinel Value**: "INITIALIZING"でrace conditionを軽減
  - kill-voice.sh実行時に50msリトライ
  - 数値PIDのバリデーション
- **複数プロジェクト対応**: 異なるプロジェクトの音声が並行再生可能

#### Phase 3: エラーハンドリングと堅牢性

- **ロッククリーンアップ**: SessionStart hookで古いロックを自動削除
  - プロセス存在確認
  - PIDファイルの整合性チェック
  - サブシェル内でロック保持（race condition対策）
- **エラーログ**: `~/.cvi/error.log`に記録
  - タイムスタンプ付きログ
  - 1MB制限でローテーション
- **依存関係チェック**: SessionStart hookで必要なコマンドを確認
  - flock, md5, say, afplay, osascript, jqをチェック
