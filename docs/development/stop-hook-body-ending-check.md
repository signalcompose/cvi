...
日本語モードでは、コードフェンスとインラインコードを除いた散文に、**ターンのどこかに**
日本語文字が含まれていればよい。CVI のルールは「日本語＝本文表示、英語＝ヒアリング用の概要」
という役割分担があるだけで、表示の順序は問わない。したがって本文が Voice 行より先でも後でも
よく、**末尾が `Voice:` 行であること自体は違反にしない**（issue #289）。ターン全体を通して
日本語の散文が一箇所も見つからない場合のみ違反とする。
