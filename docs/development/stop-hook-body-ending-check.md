...
フェンスがターン全体で閉じている場合は、結合した text ブロック全体からコードを除去するため、
ブロックをまたぐフェンスも一つのコードブロックとして扱う。閉じないフェンスが残る異常系では、
各 text ブロックから個別にコードを除去し、そのフェンス内のコードを散文と誤認せず、かつ後続の
独立した text ブロックまでコードとして隠さないようにする。
